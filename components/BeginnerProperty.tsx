...
import { SCENARIOS, simulatePreview, simulatePortfolio } from "@/lib/beginnerSim";
...
export default function BeginnerProperty({ property }: { property: Property }) {
  ...
  function buy() {
    ...
    setToast(`Bought ${property.address}. ${fmtMoney(cashIn)} cash out.`);
          ...
          {owned ? (
            ...
            <>
              ...
              <button
                onClick={buy}
                disabled={!canAfford}
                className="mt-4 w-full rounded-lg bg-[#2563EB] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#1D4ED8] disabled:opacity-50"
              >
                {canAfford
              ...
              <div className="mt-2 text-center text-[11px] text-slate-500">
                Cash available: {fmtMoney(p.cash)}
              </div>
            </>
          )}
