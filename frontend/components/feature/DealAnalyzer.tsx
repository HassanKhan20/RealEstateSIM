...
import type { Property, Strategy } from "@/shared/properties";
import {
  computeDeal,
  computeFlip,
  DEFAULT_FINANCING,
...
import { awardPropertyBoughtXp, buyProperty, getPortfolio } from "@/frontend/lib/store";
...
export default function DealAnalyzer({ property }: { property: Property }) {
  ...
  function onBuy() {
    ...
    logEvent("pro_buy", { propertyId: property.id, strategy, cashIn: deal.totalCashIn });
    setOwned(true);
    setToast(`Bought. ${fmtMoney(deal.totalCashIn)} invested. +50 XP · Go to portfolio →`);
    setTimeout(() => {
      setToast(null);
    ...
    <div className="space-y-4">
      <div className="card p-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-x-4 gap-y-1.5 border-t border-border pt-4 text-xs">
          ...
          {deal.monthlyMortgageInsurance > 0 && (
            <Row l={financingType === "fha" ? "MIP (FHA)" : "PMI"} r={fmtMoney(deal.monthlyMortgageInsurance)} />
          )}
