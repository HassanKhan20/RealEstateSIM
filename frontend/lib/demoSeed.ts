...
export function seedIfEmpty() {
  ...
  try {
    ...
    // Mark demo users as already through onboarding (Builder archetype, Level 3 worth of XP)
    if (!localStorage.getItem("onboarding-v1")) {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          archetype: "builder",
          dims: { patience: 6, risk: 0, people: 1, analytical: 4 },
          answers: [1, 1, 0, 1, 0, 1, 1],
        })
      );
    }
    if (!localStorage.getItem("progress-v1")) {
      localStorage.setItem(
        "progress-v1",
        JSON.stringify({ xp: 950, highestLevelReached: "operator" })
      );
    }
