// Thin route handler — all logic lives in backend/ai/grade.ts.
import { NextRequest } from "next/server";
import { gradeTranscript } from "@/backend/ai/grade";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const userRating = typeof body?.userRating === "number" ? body.userRating : 1000;
    const result = await gradeTranscript(body.slug, v.messages, userRating);
    if ("error" in result) {
