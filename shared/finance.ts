...
export type FinancingInputs = {
  ...
  rehab: number;
  // Mortgage insurance:
  //   FHA: 0.85% annual MIP for the life of the loan (always applied).
  //   Conventional: 0.7% PMI auto-applied if downPct < 20%, dropped once equity ≥ 20%.
  mipAnnualPct?: number; // FHA — explicit
};
...
export const DEFAULT_FINANCING: Record<FinancingType, FinancingInputs> = {
  ...
  "fha": {
    ...
    rehab: 0,
    mipAnnualPct: 0.85, // FHA MIP for the life of the loan
  },
  "hard-money": {
    type: "hard-money",
    downPaymentPct: 0.10,
...
export type DealMath = {
  ...
  monthlyHoa: number;
  monthlyMortgageInsurance: number; // PMI or FHA MIP
  monthlyPiti: number; // PI + taxes + insurance + HOA + MI
...
export function computeDeal(
  ...
  // Mortgage insurance:
  //   FHA: explicit mipAnnualPct (default 0.85%)
  //   Conventional: auto-PMI 0.7%/yr if down < 20%
  let miAnnualPct = fin.mipAnnualPct ?? 0;
  if (fin.type === "conventional" && fin.downPaymentPct < 0.20) miAnnualPct = 0.7;
  const monthlyMortgageInsurance = loanAmount > 0
    ? Math.round((loanAmount * (miAnnualPct / 100)) / 12)
    : 0;

  const monthlyPiti =
    monthlyPI + monthlyMortgageInsurance + p.monthlyTaxes + p.monthlyInsurance + p.monthlyHoa;
